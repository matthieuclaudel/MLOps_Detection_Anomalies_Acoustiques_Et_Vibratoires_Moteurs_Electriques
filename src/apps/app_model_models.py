...
class DataModel(BaseModel):
    ...
    client : Optional[str] = "user"
    versionsc : int = 0
    versionmodel : int = 0
    temp_rep : Optional[float] = 0.0
    prediction: Optional[int] = 2
    date_de_creation: datetime = datetime.utcnow()
    Mesure_CNI : Mesure
    model_config = ConfigDict(
        populate_by_name=True,
        arbitrary_types_allowed=True,
        json_encoders = {
            PyObjectId: str,  # Convertit PyObjectId en chaîne de caractères
        },
    )
class DataCollection(BaseModel):
    """
    A container holding a list of `DataModel` instances.
    This exists because providing a top-level array in a JSON response can be a [vulnerability](https://haacked.com/archive/2009/06/25/json-hijacking.aspx/)
    """

    alldata: List[DataModel]